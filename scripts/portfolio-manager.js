...
async function loadWorks(){works=await fetch('/api/projects').then(r=>r.json());$('list').innerHTML=works.map((p,i)=>'<article class="card"><small>'+esc(p.date)+(p.featured?' · 首页精选':'')+'</small><h3>'+esc(p.title)+'</h3><p>'+esc(p.company||'未填写项目')+' · '+(p.images||[]).length+' 个详情媒体</p><div class="actions"><button onclick="editWork('+i+')">编辑</button><button class="ghost danger" onclick="deleteWork('+i+')">删除</button></div></article>').join('')}
function resetEditor(){editing=null;['title','id','company','liveLink','desc'].forEach(x=>$(x).value='');$('date').value=new Date().getFullYear();$('primary').value='#28282b';$('secondary').value='#f0f4f1';$('featured').checked=true;$('cover').value='';$('bulk').value='';$('media').innerHTML=''}function newWork(){resetEditor();$('editorTitle').textContent='新增作品';addMedia();openEditor()}function editWork(i){resetEditor();editing=i;const p=works[i];$('editorTitle').textContent='编辑：'+p.title;['title','id','date','company','liveLink','primary','secondary'].forEach(x=>$(x).value=p[x]||'');$('desc').value=(p.desc||[]).join('\n\n');$('featured').checked=!!p.featured;(p.images||[]).forEach(m=>addMedia(m));openEditor()}function openEditor(){$('editor').classList.add('open');$('editor').scrollIntoView({behavior:'smooth'})}function closeEditor(){$('editor').classList.remove('open')}
